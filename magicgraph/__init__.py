...
class DiGraph(defaultdict):
  ...
  def number_of_edges(self):
    ...
    #return sum([self.degree(x) for x in self.keys()]) / 2.0
...
def from_edges(edges, undirected=False, weighted=False):
    if weighted:
        G = WeightedDiGraph()
    else:
        G = Graph()
    ...
    for edge in edges:
        if weighted:
           G[edge[0]].append(edge[1], weight=edge[2])
        else:
